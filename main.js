...
$(function() {
    ...
    //TODO - move to window.config like other UIs?
    ...
    function init(task) {
        BrainBrowser.VolumeViewer.start("view", function(viewer) {
            ...
            .then(load_nifti)
            .catch(err=>{
                console.log("trying as mask");
                fetch(config.wf_api + "/task/download/"+task._id+"?p="+encodeURIComponent(base+"mask.nii.gz")+"&at="+config.jwt)
                .then(load_nifti)
                .catch(err=>{
                    console.log("trying as t2");
                    fetch(config.wf_api + "/task/download/"+task._id+"?p="+encodeURIComponent(base+"t2.nii.gz")+"&at="+config.jwt)
                    .then(load_nifti)
                    .catch(console.error);
                });
            });
